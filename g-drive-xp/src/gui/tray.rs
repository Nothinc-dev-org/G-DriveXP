...
impl Tray for GDriveXPTray {
    ...
    fn menu(&self) -> Vec<MenuItem<Self>> {
        ...
        // Salir (apagado coordinado: igual que AppMsg::Quit — el backend en
        // main.rs ejecuta hide → unmount → marcador limpio → exit. Salir
        // directo con process::exit fingiría un crash y purgaría la caché).
        items.push(StandardItem {
            label: "Salir".to_string(),
            activate: Box::new(|_| {
                tracing::info!("👋 Cierre solicitado desde bandeja (apagado coordinado)...");
