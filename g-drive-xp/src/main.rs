...
pub fn run_backend(
    ...
    rt.block_on(async {
        ...
        let root_id = or_shutdown!(gdrive::client::DriveClient::timed("get_root_file_id", gdrive::client::CONTROL_TIMEOUT, drive_client.get_root_file_id()))
        ...
        // El manager se mueve a su factoría supervisada (un solo arranque real;
        // max_restarts=0 más abajo: ante caída, alarma ruidosa sin auto-restart
        // porque el canal de comandos y el watcher no son re-creables en caliente).
        let mut mirror_manager_opt = Some(mirror_manager);
        let mirror_factory = move || match mirror_manager_opt.take() {
            Some(m) => m.spawn(),
        ...
        // (watch retiene el valor; si falla no hay receptor = arranque roto).
        if let Err(e) = bfs_ready_tx.send(true) {
            tracing::warn!("⚠️ bfs_ready sin receptor: {}", e);
        }
        ...
        // (instancia efímera; el loop supervisado se construye en su factoría)
        ui_sender.input(gui::app_model::AppMsg::UpdateStatus("Sincronizando cambios recientes...".to_string()));
        let sync_inicial = sync::syncer::BackgroundSyncer::new(
            db.clone(),
            drive_client.clone(),
        ...
        match or_shutdown!(sync_inicial.sync_once()) {
        ...
        // Factoría supervisada: si el loop muere (panic), el supervisor lo relanza.
        let syncer_factory = {
            let db = db.clone();
            let drive_client = drive_client.clone();
            let history = history.clone();
            let sync_paused = sync_paused.clone();
            let mirror_sender = mirror_sender.clone();
            move || {
                sync::syncer::BackgroundSyncer::new(
                    db.clone(),
                    drive_client.clone(),
                    60,
                    history.clone(),
        ...
        // Fase 2.3: Uploader (subida de archivos dirty, supervisado con reinicio)
        tracing::info!("Iniciando uploader en background...");
        let uploader_factory = {
            let db = db.clone();
            let drive_client = drive_client.clone();
            let cache_dir = config.cache_dir.clone();
            let mirror_path = config.mirror_path.clone();
            let history = history.clone();
            let root_id = root_id.clone();
            move || {
                sync::uploader::Uploader::new(
                    db.clone(),
                    drive_client.clone(),
        ...
        // Fase 2.3.5: Progress Monitor (supervisado: si muere, se relanza)
        let monitor_factory = {
            let db_src = db.clone();
            let history_src = history.clone();
            move || {
                let db_monitor = db_src.clone();
                let history_monitor = history_src.clone();
                tokio::spawn(async move {
        ...
        // Fase 2.5: Servidor IPC para extensiones externas (Nautilus, supervisado)
        ...
        let ipc_factory = {
            ...
            let mirror_sender = mirror_sender.clone();
            move || {
                ipc::server::IpcServer::new(
                    ipc::get_socket_path(),
                    db.clone(),
                    mirror_path.clone(), // IPC usa rutas visibles del usuario
                    cache_dir.clone(),
                )
        ...
        // Supervisión con reinicio: ningún subsistema muere en silencio.
        // El mirror no se auto-reinicia (canal + watcher no re-creables en
        // caliente): ante caída, alarma ruidosa en logs, historial y UI.
        let mut mirror_supervised =
            utils::supervisor::SupervisedService::new("mirror", mirror_factory);
        mirror_supervised.max_restarts = 0;
        let services = vec![
            utils::supervisor::SupervisedService::new("syncer", syncer_factory),
            utils::supervisor::SupervisedService::new("uploader", uploader_factory),
            utils::supervisor::SupervisedService::new("monitor", monitor_factory),
            utils::supervisor::SupervisedService::new("ipc", ipc_factory),
            mirror_supervised,
        ];
        let sup_history = history.clone();
        let sup_ui = ui_sender.clone();
        tokio::spawn(async move {
            utils::supervisor::supervise_loop(services, move |ev| {
                use utils::supervisor::ServiceEvent::*;
                match ev {
                    Died { service, panic } => {
                        tracing::error!(
                            "💀 Servicio '{}' terminó inesperadamente (panic={})",
                            service, panic
                        );
                        // El mirror no relanza (max_restarts=0, ADR-021): no prometer "reiniciando".
                        let caida = format!("Servicio '{}' caído (panic={})", service, panic);
                        sup_history.log(
                            crate::gui::history::ActionType::Error,
                            if service == "mirror" {
                                format!("{}; usa Reiniciar para recuperarlo", caida)
                            } else {
                                format!("{}; reiniciando…", caida)
                            },
                        );
                        sup_ui.input(crate::gui::app_model::AppMsg::UpdateStatus(
                            if service == "mirror" {
                                format!("🛑 {} detenido; usa Reiniciar para recuperarlo", service)
                            } else {
                                format!("⚠️ {} falló; reiniciando…", service)
                            },
                        ));
                    }
                    Restarted { service, attempt } => {
                        tracing::warn!(
                            "🔄 Servicio '{}' relanzado (fallo consecutivo nº {})",
                            service, attempt
                        );
                    }
                    GaveUp { service } => {
                        tracing::error!(
                            "🛑 Servicio '{}' superó reintentos: requiere intervención manual",
                            service
                        );
                        // El mirror no tiene auto-restart (ADR-021): latchar el
                        // degradado para que la GUI lo muestre con botón Reiniciar.
                        if service == "mirror" {
                            crate::mirror::MIRROR_DEGRADED.store(true, Ordering::SeqCst);
                        }
                        sup_history.log(
                            crate::gui::history::ActionType::Error,
                            if service == "mirror" {
                                format!("Servicio '{}' detenido; usa Reiniciar para recuperarlo", service)
                            } else {
                                format!("Servicio '{}' detenido tras reintentos", service)
                            },
                        );
                        sup_ui.input(crate::gui::app_model::AppMsg::UpdateStatus(format!(
                            "🛑 {} detenido; revisa logs",
                            service
                        )));
                    }
                }
        ...
        if let Err(e) = mirror_sender.send(mirror::MirrorCommand::Shutdown).await {
            tracing::debug!("Shutdown al mirror no entregado (ya caído): {}", e);
        }
