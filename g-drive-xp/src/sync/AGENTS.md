...
| `uploader.rs`  | `Uploader`: escanea `sync_state WHERE dirty=1` y `local_sync_files WHERE dirty=1`. Sube archivos via Resumable Upload con exponential backoff. Permanente clasificado (`DriveError::permanent_of`) → reconcilia o cuarentena en `failed_changes` (`up:*`/`uplocal:*`, umbral 10); transitorio → backoff por archivo en memoria, dirty intacto. |
