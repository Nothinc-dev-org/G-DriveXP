...
pub async fn bootstrap_level1(
    ...
    let root_children = DriveClient::timed("list_root_children", CONTROL_TIMEOUT, client.list_root_children(root_id)).await?;
...
pub async fn bootstrap_remaining_bfs(
    ...
    loop {
        ...
        let (page_files, next_token) = DriveClient::timed("fetch_files_page", CONTROL_TIMEOUT, client.fetch_files_page(page_token.as_deref())).await?;
...
pub async fn repair_ownership_metadata(
    ...
    let files = DriveClient::timed("list_all_files", CONTROL_TIMEOUT, client.list_all_files()).await?;
    let total = files.len();
