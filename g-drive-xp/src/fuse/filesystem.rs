...
impl Filesystem for GDriveFS {
    ...
    async fn open(&self, _req: Request, inode: u64, flags: u32) -> Result<ReplyOpen> {
        ...
        // Workspace (Docs/Sheets/...): no aceptan contenido binario (la API lo
        // rechaza). Rechazar la apertura de escritura aquí con EROFS (fail fast:
        // el editor lo ve read-only) en vez de aceptar writes que el uploader
        // reintentaría eternamente.
        if is_workspace && write_intent(flags) {
            tracing::debug!("🚫 open() escritura rechazada para Workspace inode={} mime={}", inode, attrs.mime_type.as_deref().unwrap_or("?"));
    ...
    async fn write(
        ...
        // Defensa en profundidad: si algo llegara a escribir por otra vía, el
        // write a un Workspace se rechaza aquí (nunca generar dirty imposible
        // de subir).
        let mime: Option<String> = sqlx::query_scalar("SELECT mime_type FROM attrs WHERE inode = ?")
            .bind(inode as i64)
            .fetch_optional(self.db.pool())
            .await
            .map_err(|e| {
                error!("Error obteniendo mime: {}", e);
                Errno::from(libc::EIO)
            })?;
        if mime.as_deref().map(shortcuts::is_workspace_file).unwrap_or(false) {
            return Err(Errno::from(libc::EROFS));
        }
    ...
    async fn setattr(
        ...
        if let Some(size) = set_attr.size {
            // Truncar Workspace también es escritura: rechazar (mismo loop
            // eterno del uploader que un write).
            let trunc_mime: Option<String> = sqlx::query_scalar("SELECT mime_type FROM attrs WHERE inode = ?")
                .bind(inode as i64)
                .fetch_optional(self.db.pool())
                .await
                .map_err(|_| Errno::from(libc::EIO))?;
            if trunc_mime.as_deref().map(shortcuts::is_workspace_file).unwrap_or(false) {
                return Err(Errno::from(libc::EROFS));
            }
            // Truncar archivo
            let gdrive_id = sqlx::query_scalar::<_, String>("SELECT gdrive_id FROM inodes WHERE inode = ?")
            ...
            if cache_path.exists() {
                ...
                // Archivo disperso del tamaño pedido: set_len reserva el tamaño
                // lógico al instante sin materializar ni un byte en RAM
                // (antes: vec![0u8; size] → OOM con un truncate gigante,
                // alcanzable con un simple `truncate -s 100G`).
    ...
    async fn rename(
        ...
        // Anti-ciclo: mover un nodo bajo sí mismo o bajo su descendiente
        // crearía un ciclo en dentry (loops infinitos en resolver/CTEs).
        if self.db.is_descendant_or_self(inode, new_parent).await
            .map_err(|_| Errno::from(libc::EIO))?
        {
            tracing::warn!("⛔ rename rechaza mover inode={} bajo su descendiente new_parent={}", inode, new_parent);
...
impl GDriveFS {
    ...
    async fn ensure_range_cached(
        ...
        let download_tasks: Vec<_> = missing_ranges.into_iter().map(|(start, end)| {
            ...
            tokio::spawn(async move {
                ...
                // Descargar chunk
                let data = DriveClient::timed("download_chunk", CHUNK_TIMEOUT, drive_client.download_chunk(&gdrive_id, start, chunk_size)).await?;
            ...
            match result {
                ...
                Ok(Err(e)) => {
                    ...
                    if err_msg.contains("416") {
                        ...
                        if let Ok(remote_file) = DriveClient::timed("get_file_metadata", CONTROL_TIMEOUT, self.drive_client.get_file_metadata(gdrive_id)).await {
    ...
    async fn prefetch_entire_file(
        ...
        if file_size < SINGLE_DOWNLOAD_THRESHOLD {
            ...
            let data = DriveClient::timed("download_chunk", CHUNK_TIMEOUT, drive_client.download_chunk(gdrive_id, 0, file_size as u32)).await?;
            ...
            for batch in pending_chunks.chunks(MAX_CONCURRENT) {
                ...
                for &(start, end) in batch {
                    ...
                    download_tasks.push(tokio::spawn(async move {
                        ...
                        let data = DriveClient::timed("download_chunk", CHUNK_TIMEOUT, client_clone.download_chunk(&gdrive_id_clone, start, m_size)).await?;
...
/// ¿Estos flags de open() implican intención de escritura?
/// O_RDONLY=0 → accmode 0 es lectura; WRONLY/RDWR (o TRUNC/CREAT/APPEND, que
/// solo tienen sentido escribiendo) → true.
...
#[cfg(test)]
mod tests {
    use super::write_intent;

    /// write_intent decide qué flags de open() implican escritura.
    #[test]
    fn write_intent_solo_lectura_no_escribe() {
        assert!(!write_intent(libc::O_RDONLY as u32));
    }

    #[test]
    fn write_intent_escritura_y_lectoescritura_si() {
        assert!(write_intent(libc::O_WRONLY as u32));
        assert!(write_intent(libc::O_RDWR as u32));
        assert!(write_intent(libc::O_WRONLY as u32 | libc::O_CREAT as u32));
        assert!(write_intent(libc::O_RDWR as u32 | libc::O_TRUNC as u32));
        assert!(write_intent(libc::O_WRONLY as u32 | libc::O_APPEND as u32));
    }
    /// El truncate debe ser disperso e instantáneo: set_len, jamás
    /// materializar ceros en RAM (es lo que usa setattr para caché nuevo).
    #[test]
    fn truncate_es_disperso_sin_materializar() {
        let dir = std::env::temp_dir().join("gdrivexp-sparse-test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let p = dir.join("f");
        std::fs::File::create(&p)
            .and_then(|f| f.set_len(10 * 1024 * 1024 * 1024))
            .unwrap();
        let m = std::fs::metadata(&p).unwrap();
        assert_eq!(m.len(), 10 * 1024 * 1024 * 1024);
        // Contenido lógico: ceros sin haberlos escrito.
