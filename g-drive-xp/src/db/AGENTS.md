...
- **FKs**: sqlx activa `foreign_keys=ON` por defecto en cada conexión (no hay PRAGMA en el repo porque no hace falta). `hard_delete_subtree_tx` borra subárboles hojas-primero en una tx (las tablas viejas no tienen CASCADE: borrar un dir con hijos vivos fallaría); `inode=1` rechazado ruidosamente; la purga no aborta ante un item patológico.
